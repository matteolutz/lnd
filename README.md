...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create lnd-ff79
  fly apps create lnd-ff79-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app lnd-ff79
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app lnd-ff79-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'lnd-ff79-staging' does not match app name in config file 'lnd-ff79'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name lnd-ff79-db
  fly postgres attach --app lnd-ff79 lnd-ff79-db

  fly postgres create --name lnd-ff79-staging-db
  fly postgres attach --app lnd-ff79-staging lnd-ff79-staging-db
  ```
